  ...
  if($dummy == 'core team') {
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            $response['member_photo_url'] = "https://dscgmrit.herokuapp.com/admin/team/".$row['member_photo'];
            $response['member_designation'] = $row['member_designation'];
            $response['member_department'] = $row['member_dept'];
            ...
            $response['linkedin_profile'] = $row['linkedlin_profile'];
